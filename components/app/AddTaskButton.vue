    ...
    <div class=" add-task" :class="[isOpen ? 'block' : 'none']">
      ...
      <div class="btn-group">
        <button class="add-today-btn" id="today-add" @click="dateModal">
          <img src="/sm_cal_green.svg" /> Today
        </button>
        <button class="add-inbox-btn">
          <img src="/sm-inb.svg" /> Inbox
        </button>
      </div>
    </div>
    <div class="cancel-btn-group" :class="[isOpen ? 'flex' : 'none']">
      <button class="add-task-btn-1" :class="{' content-empty' : isContentEmpty}" @click="addTask()">Add Task</button>
  ...
  methods: {
    ...
    addTask() {
     if (this.value.content !== ''){
       this.$store.commit('addTask', {value:{...this.value}, id:this.id});
